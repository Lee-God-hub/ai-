...
const aiApi = {
   ...
   */
  askQuestion(data) {
    return api_request.request.post("/api/ai/qa/ask", data, {
      loadingText: "AI正在思考中..."
    });
  },
  /**
  ...
  getQAHistory(limit = 20) {
    return api_request.request.get("/api/ai/qa/history", { limit }, {
      loading: false
    });
  },
  // ==================== 文案生成相关接口 ====================
  ...
  generateContent(data) {
    return api_request.request.post("/api/ai/content/generate", data, {
  ...
  markContentAsApplied(generationId, propertyId) {
    return api_request.request.post(`/api/ai/content/apply?generationId=${generationId}&propertyId=${propertyId}`, {});
  },
  /**
   * 获取房东的文案生成历史
  ...
  getContentHistory(limit = 10) {
    return api_request.request.get("/api/ai/content/history", { limit });
  },
  // ==================== 智能推荐相关接口 ====================
  /**
   * 获取智能推荐房源列表
  ...
  getRecommendations(limit = 10) {
    return api_request.request.get("/api/ai/recommend/properties", { limit }, {
  ...
  recordRecommendClick(data) {
    return api_request.request.post("/api/ai/recommend/click", data, {
      loading: false
    });
  },
  /**
  ...
  recordUserBehavior(propertyId, behaviorType) {
    return api_request.request.post(`/api/ai/behavior/record?propertyId=${propertyId}&behaviorType=${behaviorType}`, {}, {
      loading: false
    });
  },
  /**
   ...
   */
  healthCheck() {
    return api_request.request.get("/api/ai/health", {}, {
      loading: false
    });
  }
};
